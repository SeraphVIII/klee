...
std::vector<klee::mapofsets::DiskMapOfSets::Entry> DiskMapOfSets::subsets(const std::set<std::string>& query_set) {
...
void DiskMapOfSets::find_subsets(uint32_t node_id, std::set<std::string> accum,
  ...
  const auto& node = get_node(node_id);
  if (node.is_end_of_set()) {
    klee::mapofsets::DiskMapOfSets::Entry e{accum, read_value(node.value_offset())};
    results.push_back(e);
  }
  ...
  for (auto q_it = q_begin; q_it != q_end; ++q_it) {
    ...
    auto child_it = std::lower_bound(node.children().begin(), node.children().end(), elt,
      [this](const Child& c, const std::string& t){ return deserialize_key(c.key()) < t; });
...
std::vector<klee::mapofsets::DiskMapOfSets::Entry> DiskMapOfSets::supersets(const std::set<std::string>& query_set) {
  std::vector<klee::mapofsets::DiskMapOfSets::Entry> results;
  std::set<std::string> accum;
